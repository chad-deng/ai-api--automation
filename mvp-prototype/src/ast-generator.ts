import { Project, SourceFile, VariableDeclarationKind, Writers, ScriptTarget, ModuleKind } from 'ts-morph';
import { EndpointInfo } from './analyzer';
import { SchemaAwareDataGenerator } from './data-generator';
import { OpenAPISpec } from './parser';

export class ASTGenerator {
  private project: Project;
  private dataGenerator: SchemaAwareDataGenerator;
  ...
  constructor(spec: OpenAPISpec) {
    ...
    this.dataGenerator = new SchemaAwareDataGenerator(spec);
  }
  ...
  private addImports(sourceFile: SourceFile, hasAuth: boolean): void {
  ...
  private generateEndpointTest(writer: any, endpoint: EndpointInfo, hasAuth: boolean): void {
  ...
  private generatePositiveTest(writer: any, endpoint: EndpointInfo, method: string, testPath: string, hasAuth: boolean): void {
  ...
  private generateNegativeTests(writer: any, endpoint: EndpointInfo, method: string, testPath: string, hasAuth: boolean): void {
    const hasBody = ['post', 'put', 'patch'].includes(method);
  ...
  private processPathForTesting(path: string, parameters?: any[]): string {
  ...
  private generateTestValueForParameter(param: any): any {
    if (param.schema) {
  ...
  private generateTestDataForEndpoint(endpoint: EndpointInfo): any | null {
    ...
    if (hasAuth) {
      ...
      sourceFile.addFunction({
        ...
        statements: writer => {
          ...
          const hasBearer = Object.values(authSchemes).some((scheme: any) => scheme.type === 'http' && scheme.scheme === 'bearer');
          const hasBasic = Object.values(authSchemes).some((scheme: any) => scheme.type === 'http' && scheme.scheme === 'basic');
