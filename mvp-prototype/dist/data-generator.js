...
exports.SchemaAwareDataGenerator = void 0;
class SchemaAwareDataGenerator {
    constructor(spec) {
        this.spec = spec;
    }
    generateTestData(schemaRef, config = { useRealistic: true, includeBoundaryValues: false, generateInvalidData: false }) {
        if (typeof schemaRef === 'string' && schemaRef.startsWith('#/')) {
            // Resolve schema reference
            const refPath = schemaRef.replace('#/', '').split('/');
            let schema = this.spec;
    ...
    generateObjectData(schema, config) {
        const result = {};
    ...
    generateArrayData(schema, config) {
    ...
    generateStringData(schema, config) {
    ...
    generateNumberData(schema, config) {
    ...
    generateBooleanData(schema, config) {
    ...
    generateInvalidTestData(schemaRef) {
    ...
    generateInvalidFromSchema(schema) {
        if (!schema || !schema.type) {
            return null;
        }
        switch (schema.type) {
...
exports.SchemaAwareDataGenerator = SchemaAwareDataGenerator;
