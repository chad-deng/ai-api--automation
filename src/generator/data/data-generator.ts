...
import { Schema, Parameter, OpenAPISpec } from '../../types';
...
export class DataGenerator {
  ...
  private rng: () => number;
  private generationStack: string[] = [];
  ...
  async generateFromSchema(
    schema: Schema, 
    type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal' = 'valid'
   ...
   */
  async generateParameterData(parameter: Parameter, type: 'valid' | 'invalid' | 'edge' = 'valid'): Promise<any> {
   ...
   */
  async generateRequestBodyData(
    requestBody: any, 
    contentType: string = 'application/json',
    type: 'valid' | 'invalid' | 'edge' = 'valid'
  ): Promise<any> {
  ...
  async generateBatch(
    schema: Schema, 
    count: number, 
    types: ('valid' | 'invalid' | 'edge' | 'minimal' | 'maximal')[] = ['valid']
  ): Promise<DataGenerationResult[]> {
   ...
   */
  private async generateValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any> {
   ...
   */
  private generateStringValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): string {
    if (type === 'invalid') {
   ...
   */
  private generateNumberValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): number {
    if (type === 'invalid') {
   ...
   */
  private generateIntegerValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): number {
    if (type === 'invalid') {
   ...
   */
  private generateBooleanValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): boolean | any {
    if (type === 'invalid') {
   ...
   */
  private async generateArrayValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any[]> {
    if (type === 'invalid') {
      return 'not-an-array' as any; // Invalid array
    }
    ...
    for (let i = 0; i < targetLength; i++) {
      ...
      const validType: 'valid' | 'edge' | 'minimal' | 'maximal' = type === 'invalid' ? 'valid' : (type as any);
      const item = await this.generateValue(itemSchema as Schema, validType);
      items.push(item);
   ...
   */
  private async generateObjectValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any> {
    if (type === 'invalid') {
    ...
    for (const propertyName of requiredFields) {
      if (properties[propertyName]) {
        ...
        // @ts-ignore - intentional type comparison for data generation
      const validType: 'valid' | 'edge' | 'minimal' | 'maximal' = type === 'invalid' ? 'valid' : (type as any);
        obj[propertyName] = await this.generateValue(
          properties[propertyName] as Schema, 
          validType
        );
        this.generationStack.pop();
      }
    ...
    if (type !== 'minimal') {
      let propertiesToGenerate: string[];
   ...
   */
  private generateFormattedString(format: string, schema: Schema, type: string): string {
    switch (format) {
      case 'email':
  ...
  private generatePatternString(pattern: string, schema: Schema, type: string): string {
  ...
  private generateInvalidString(schema: Schema): any {
  ...
  private generateRandomString(length: number, charset: string = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'): string {
    let result = '';
   ...
   */
  private async generateAllOfValue(schemas: Schema[], type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any> {
  ...
  private async generateOneOfValue(schemas: Schema[], type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any> {
    // Pick one schema randomly and generate from it
    const selectedSchema = schemas[Math.floor(this.rng() * schemas.length)];
    if (!selectedSchema) throw new Error('No schema available in oneOf');
    return this.generateValue(selectedSchema, type);
  }

  private async generateAnyOfValue(schemas: Schema[], type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): Promise<any> {
    // Similar to oneOf for now
    const selectedSchema = schemas[Math.floor(this.rng() * schemas.length)];
    if (!selectedSchema) throw new Error('No schema available in anyOf');
    return this.generateValue(selectedSchema, type);
  }

  private generateUnknownTypeValue(schema: Schema, type: 'valid' | 'invalid' | 'edge' | 'minimal' | 'maximal'): any {
  ...
  private extractConstraints(schema: Schema): string[] {
    ...
    if (schema.required) constraints.push(`required: [${schema.required.join(', ')}]`);
  ...
  public updateOptions(options: Partial<DataGenerationOptions>): void {
