...
import { ParsedOpenAPI, SecurityScheme } from '../types';
...
export class SecurityValidator {
   ...
   */
  async validateApiSecurity(spec: ParsedOpenAPI): Promise<SecurityValidationResult> {
   ...
   */
  async generateSecurityTests(spec: ParsedOpenAPI): Promise<SecurityTestCase[]> {
   ...
   */
  async validateAuthConfig(config: AuthConfiguration): Promise<SecurityValidationResult> {
    const vulnerabilities: SecurityVulnerability[] = [];
    const recommendations: string[] = [];
  ...
  private async validateGlobalSecurity(
    spec: ParsedOpenAPI, 
    vulnerabilities: SecurityVulnerability[], 
  ...
  private async validateEndpointSecurity(
    spec: ParsedOpenAPI, 
    vulnerabilities: SecurityVulnerability[], 
    _recommendations: string[]
  ): Promise<void> {
  ...
  private async validateSecuritySchemes(
    spec: ParsedOpenAPI, 
    vulnerabilities: SecurityVulnerability[], 
    _recommendations: string[]
  ): Promise<void> {
  ...
  private async validateCommonIssues(
    spec: ParsedOpenAPI, 
    vulnerabilities: SecurityVulnerability[], 
    _recommendations: string[]
  ): Promise<void> {
    ...
    const operations = await this.parser.extractOperations(spec);
    const hasRateLimitHeaders = operations.some(({ operation }) => 
      operation.responses && Object.values(operation.responses).some(response => 
        typeof response === 'object' && response.headers && 
        Object.keys(response.headers).some(header => 
          header.toLowerCase().includes('rate') || header.toLowerCase().includes('limit')
        )
      )
    );
  ...
  private async generateAuthenticationTests(operation: any, method: string, path: string): Promise<SecurityTestCase[]> {
  ...
  private async generateAuthorizationTests(operation: any, method: string, path: string): Promise<SecurityTestCase[]> {
    const tests: SecurityTestCase[] = [];
  ...
  private async generateInjectionTests(operation: any, method: string, path: string): Promise<SecurityTestCase[]> {
    const tests: SecurityTestCase[] = [];
  ...
  private async generateDisclosureTests(operation: any, method: string, path: string): Promise<SecurityTestCase[]> {
    const tests: SecurityTestCase[] = [];
  ...
  private validateOAuth2Flows(flows: any, schemeName: string, vulnerabilities: SecurityVulnerability[]): void {
    if (flows.implicit) {
      vulnerabilities.push({
        type: 'medium',
        category: 'Authentication',
  ...
  private isInsecureScheme(scheme: SecurityScheme): boolean {
    if (typeof scheme === 'object') {
  ...
  private mightExposeSensitiveData(operation: any, path: string): boolean {
