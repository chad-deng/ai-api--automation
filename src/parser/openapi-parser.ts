...
export class OpenAPIParser {
  ...
  private readonly supportedVersions = ['3.0.0', '3.0.1', '3.0.2', '3.0.3', '3.1.0'];
   ...
   */
  async parseFromFile(filePath: string, options: ParsingOptions = {}): Promise<ParsingResult> {
    ...
    try {
      ...
      if (!exists) {
        throw new ParsingError(`OpenAPI spec file not found: ${filePath}`);
      }
   ...
   */
  async parseContent(content: string, filePath?: string): Promise<OpenAPISpec> {
    try {
      let spec: any;
      ...
      if (!spec || typeof spec !== 'object') {
        throw new ParsingError('Invalid OpenAPI specification: not a valid object');
      }

      if (!spec.openapi) {
        throw new ParsingError('Invalid OpenAPI specification: missing "openapi" field');
      }

      if (!this.supportedVersions.includes(spec.openapi)) {
        throw new ParsingError(`Unsupported OpenAPI version: ${spec.openapi}. Supported: ${this.supportedVersions.join(', ')}`);
      }
      ...
      throw new ParsingError(`Failed to parse OpenAPI specification: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async validateSpec(spec: OpenAPISpec): Promise<{
    isValid: boolean;
    ...
    if (spec.paths) {
      for (const [pathStr, pathItem] of Object.entries(spec.paths)) {
        ...
        if (pathItem && typeof pathItem === 'object') {
          await this.validatePathItem(pathStr, pathItem as PathItem, errors, warnings);
        }
      }
    }
   ...
   */
  async enrichSpecification(spec: OpenAPISpec, _options: ParsingOptions): Promise<ParsedOpenAPI> {
    const enriched: ParsedOpenAPI = {
      ...spec,
      metadata: {
        totalPaths: 0,
        totalOperations: 0,
   ...
   */
  async extractOperations(spec: ParsedOpenAPI): Promise<Array<{
    path: string;
    method: string;
    operation: Operation;
    pathItem: PathItem;
  ...
  private async validatePathItem(pathStr: string, pathItem: PathItem, errors: ValidationErrorDetail[], warnings: string[]): Promise<void> {
    const httpMethods = ['get', 'post', 'put', 'delete', 'patch', 'head', 'options', 'trace'];
  ...
  private async validateOperation(pathStr: string, method: string, operation: Operation, errors: ValidationErrorDetail[], warnings: string[]): Promise<void> {
  ...
  private async validateComponents(components: any, errors: ValidationErrorDetail[], _warnings: string[]): Promise<void> {
    // Validate schemas
    ...
    if (components.securitySchemes) {
      for (const [schemeName, scheme] of Object.entries(components.securitySchemes)) {
        if (!scheme || typeof scheme !== 'object' || !(scheme as any).type) {
          errors.push({
  ...
  private async validateSecurity(security: any[], securitySchemes: any, errors: ValidationErrorDetail[], _warnings: string[]): Promise<void> {
    if (!securitySchemes) {
      errors.push({
        field: 'security',
