...
export class OpenAPIValidator {
   ...
   */
  async validateFromFile(filePath: string, options: ValidationOptions = {}): Promise<ValidationResult> {
    try {
      // Parse the specification first
      const parsingOptions: any = {};
   ...
   */
  async validateSpec(spec: ParsedOpenAPI, options: ValidationOptions = {}): Promise<ValidationResult> {
   ...
   */
  private async validateSchema(spec: ParsedOpenAPI): Promise<{
    isValid: boolean;
   ...
   */
  private async validatePaths(spec: ParsedOpenAPI): Promise<{
    isValid: boolean;
    errors: ValidationErrorDetail[];
    warnings: string[];
  }> {
    ...
    for (const [pathStr, pathItem] of Object.entries(spec.paths)) {
      ...
      if (pathItem && typeof pathItem === 'object') {
        ...
        for (const param of pathParams) {
          const isDocumented = pathItemObj.parameters?.some(p => 
            typeof p === 'object' && 'name' in p && p.name === param && p.in === 'path'
          );
          if (!isDocumented) {
   ...
   */
  private async validateOperations(spec: ParsedOpenAPI): Promise<{
    isValid: boolean;
    errors: ValidationErrorDetail[];
    warnings: string[];
  }> {
    ...
    if (!spec.paths) {return { isValid: true, errors, warnings };}
   ...
   */
  private async validateResponses(spec: ParsedOpenAPI): Promise<{
    isValid: boolean;
    errors: ValidationErrorDetail[];
    warnings: string[];
  }> {
    ...
    for (const [pathStr, pathItem] of Object.entries(spec.paths)) {
      ...
      for (const method of httpMethods) {
        ...
        for (const [statusCode, response] of Object.entries(operation.responses)) {
          ...
          if (typeof response !== 'object') {continue;}
   ...
   */
  private async validateCustomRules(spec: ParsedOpenAPI, options: ValidationOptions): Promise<{
    errors: string[];
    warnings: string[];
   ...
   */
  private async validateSecurity(spec: ParsedOpenAPI): Promise<{
    errors: string[];
    warnings: string[];
    scoreDeduction: number;
  }> {
  ...
  private async validateSchemaObject(schema: Schema, fieldPath: string): Promise<ValidationErrorDetail[]> {
    const errors: ValidationErrorDetail[] = [];
