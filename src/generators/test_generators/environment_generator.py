...
'''
    ...
    def _generate_environment_summary_test(self, api_spec: Dict[str, Any], tests: List[EnvironmentTest]) -> str:
        ...
        # Use string concatenation instead of complex f-string to avoid issues
        content = f'''    @pytest.mark.asyncio
    @pytest.mark.environment
    async def test_{operation_id}_environment_summary(self, client, auth_headers, env_backup):
        ...
        try:
            ...
            response = await client.{method.lower()}("{path}", json=test_payload, headers=auth_headers)
