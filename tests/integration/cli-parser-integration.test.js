...
describe('CLI Parser Integration', () => {
    ...
    describe('Generate Command with Parser', () => {
        test('should successfully parse OpenAPI spec for generation', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('📖 Parsing OpenAPI specification...'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Found 4 operations in 3 paths'));
        });
        test('should show parsed API information', async () => {
            const result = await cli.run(['generate', petstoreFixture, '--verbose']);
            expect(result.success).toBe(true);
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('HTTP methods: PUT, POST, GET'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Extracted 4 operations for test generation'));
        });
    ...
    describe('Validate Command with Validator', () => {
        test('should successfully validate valid OpenAPI spec', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('🔍 Parsing OpenAPI specification...'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('✅ OpenAPI specification is valid!'));
        });
        test('should detect validation errors in invalid spec', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('❌ OpenAPI specification has issues:'));
        });
    ...
    describe('Output Formatting Integration', () => {
        test('should format generation output correctly', async () => {
            ...
            expect(result.success).toBe(true);
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('✅ Generation completed successfully!'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Tests created: 4'));
        });
...
//# sourceMappingURL=cli-parser-integration.test.js.map