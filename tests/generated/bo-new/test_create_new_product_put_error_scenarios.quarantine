"""
Error Scenarios Test Suite for Create new product
...
class TestCreatenewproductErrorScenarios:
    """
    Error scenario test suite for Create new product
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.put(
            "/api/v2/products/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v2/products/../../etc/passwd",  # Path traversal
            "/api/v2/products/<script>alert(1)</script>",  # XSS attempt
            "/api/v2/products/';DROP TABLE users;--",  # SQL injection
        ]
