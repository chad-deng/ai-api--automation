"""
Error Scenarios Test Suite for Get merchant information
...
class TestGetmerchantinformationErrorScenarios:
    """
    Error scenario test suite for Get merchant information
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.get(
            "/api/v2/merchant/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_bad_request(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v2/merchant?invalid_param=value",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v2/merchant/../../etc/passwd",  # Path traversal
            "/api/v2/merchant/<script>alert(1)</script>",  # XSS attempt
            "/api/v2/merchant/';DROP TABLE users;--",  # SQL injection
        ]
