"""
Error Scenarios Test Suite for Delete product
...
class TestDeleteproductErrorScenarios:
    """
    Error scenario test suite for Delete product
    """
    ...
    async def test_method_not_allowed(self, async_client, auth_headers):
        ...
        invalid_method = "DELETE" if "DELETE" != "DELETE" else "PATCH"
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.delete(
            "/api/v2/products/{id}/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_bad_request(self, async_client, auth_headers):
        ...
        response = await async_client.delete(
            "/api/v2/products/{id}?invalid_param=value",
            headers=auth_headers['valid']
        )
    ...
    async def test_unsupported_media_type(self, async_client, auth_headers):
        ...
        pytest.skip("Not applicable for DELETE")
    ...
    async def test_timeout_handling(self, async_client, auth_headers):
        ...
        try:
            response = await slow_client.delete(
                "/api/v2/products/{id}",
                headers=auth_headers['valid']
            )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v2/products/{id}/../../etc/passwd",  # Path traversal
            "/api/v2/products/{id}/<script>alert(1)</script>",  # XSS attempt
            "/api/v2/products/{id}/';DROP TABLE users;--",  # SQL injection
        ]
