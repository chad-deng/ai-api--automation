...
# Basic Concurrency Tests for: Searches purchase orders based on provided criteria
# Method: POST
# Path: /api/v3/inventory/purchase-order/search
...
class TestConcurrencyEndpoint:
    ...
    async def test_search_purchase_orders_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/api/v3/inventory/purchase-order/search", json=payload, headers=auth_headers)
