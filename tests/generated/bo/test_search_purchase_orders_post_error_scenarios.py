"""
Error Scenarios Test Suite for Search purchase orders
...
class TestSearchpurchaseordersErrorScenarios:
    """
    Error scenario test suite for Search purchase orders
    """
    ...
    async def test_method_not_allowed(self, async_client, auth_headers):
        ...
        invalid_method = "DELETE" if "POST" != "DELETE" else "PATCH"
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.post(
            "/api/v3/inventory/purchase-order/search/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/inventory/purchase-order/search/../../etc/passwd",  # Path traversal
            "/api/v3/inventory/purchase-order/search/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/inventory/purchase-order/search/';DROP TABLE users;--",  # SQL injection
        ]
