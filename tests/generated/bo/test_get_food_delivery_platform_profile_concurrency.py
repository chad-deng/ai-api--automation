...
# Basic Concurrency Tests for: Retrieves food delivery platform profile information
# Method: GET
# Path: /api/v3/addons/food-delivery/{platform}
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_food_delivery_platform_profile_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/api/v3/addons/food-delivery/{platform}", json=payload, headers=auth_headers)
