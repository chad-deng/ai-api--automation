...
# Basic Concurrency Tests for: Retrieves a specific product by its ID
# Method: GET
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_product_by_id_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/api/v2/products/{id}", json=payload, headers=auth_headers)
