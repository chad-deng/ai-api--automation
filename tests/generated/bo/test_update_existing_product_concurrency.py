...
# Basic Concurrency Tests for: Updates an existing product
# Method: POST
...
class TestConcurrencyEndpoint:
    ...
    async def test_update_existing_product_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/api/v2/products/{id}", json=payload, headers=auth_headers)
