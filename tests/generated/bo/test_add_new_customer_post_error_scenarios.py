"""
Error Scenarios Test Suite for Add new customer
...
class TestAddnewcustomerErrorScenarios:
    """
    Error scenario test suite for Add new customer
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.post(
            "/api/v3/customer/customer/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/customer/customer/../../etc/passwd",  # Path traversal
            "/api/v3/customer/customer/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/customer/customer/';DROP TABLE users;--",  # SQL injection
        ]
