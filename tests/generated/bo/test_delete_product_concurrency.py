...
# Basic Concurrency Tests for: Deletes a product by its ID
# Method: DELETE
...
class TestConcurrencyEndpoint:
    ...
    async def test_delete_product_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.delete("/api/v2/products/{id}", json=payload, headers=auth_headers)
