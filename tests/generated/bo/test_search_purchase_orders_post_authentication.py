"""
Authentication & Authorization Test Suite for Search purchase orders
...
class TestSearchpurchaseordersAuthentication:
    """
    ...
    async def test_authentication_required(self, async_client):
        ...
        response = await async_client.post(
            "/api/v3/inventory/purchase-order/search",
    ...
    async def test_role_based_access_control(self, async_client, test_roles):
        ...
        for role_name, role_data in test_roles.items():
            ...
            if "post" == "get" and "read" in role_data["permissions"]:
                # Should have read access
                assert response.status_code in [200, 403], \
                    f"Role {role_name} read access unexpected: {response.status_code}"
            elif "post" in ["post", "put", "patch"] and "write" in role_data["permissions"]:
                # Should have write access
                assert response.status_code in [200, 201, 403], \
                    f"Role {role_name} write access unexpected: {response.status_code}"
            elif "post" == "delete" and "delete" in role_data["permissions"]:
    ...
    async def test_resource_ownership_validation(self, async_client, valid_token):
        ...
        other_user_resource = "/api/v3/inventory/purchase-order/search/other_user_123"
    ...
    async def test_cors_policy_enforcement(self, async_client):
        ...
        if response.status_code in [200, 204]:
            ...
            assert "POST" in allowed_methods.upper(), \
                f"Method POST not in CORS allowed methods: {allowed_methods}"
    ...
    async def test_concurrent_session_handling(self, async_client, valid_token):
        ...
        tasks = [
            async_client.post("/api/v3/inventory/purchase-order/search", headers=headers)
            for _ in range(5)
        ]
    ...
    async def test_session_timeout(self, async_client, valid_token):
        ...
        later_response = await async_client.post(
            "/api/v3/inventory/purchase-order/search",
            headers=headers
        )
