...
# Basic Concurrency Tests for: Returns the health status of the API
# Method: GET
# Path: /health
...
class TestConcurrencyEndpoint:
    ...
    async def test_health_check_endpoint_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/health", json=payload, headers=auth_headers)
