"""
Error Scenarios Test Suite for Update subscription
...
class TestUpdatesubscriptionErrorScenarios:
    """
    Error scenario test suite for Update subscription
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.post(
            "/api/v3/subscription/update/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/subscription/update/../../etc/passwd",  # Path traversal
            "/api/v3/subscription/update/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/subscription/update/';DROP TABLE users;--",  # SQL injection
        ]
