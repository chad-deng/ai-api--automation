...
# Basic Concurrency Tests for: Updates the current subscription
# Method: POST
# Path: /api/v3/subscription/update
...
class TestConcurrencyEndpoint:
    ...
    async def test_update_subscription_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/api/v3/subscription/update", json=payload, headers=auth_headers)
