"""
Error Scenarios Test Suite for Export goods report
...
class TestExportgoodsreportErrorScenarios:
    """
    Error scenario test suite for Export goods report
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.post(
            "/api/v3/stocks/export-goods-report/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/stocks/export-goods-report/../../etc/passwd",  # Path traversal
            "/api/v3/stocks/export-goods-report/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/stocks/export-goods-report/';DROP TABLE users;--",  # SQL injection
        ]
