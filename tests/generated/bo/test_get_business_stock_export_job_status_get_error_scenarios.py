"""
Error Scenarios Test Suite for Get business stock export job status
...
class TestGetbusinessstockexportjobstatusErrorScenarios:
    """
    Error scenario test suite for Get business stock export job status
    """
    ...
    async def test_method_not_allowed(self, async_client, auth_headers):
        ...
        invalid_method = "DELETE" if "GET" != "DELETE" else "PATCH"
        
        response = await async_client.request(
            invalid_method.lower(),
            "/api/v3/stocks/goods-report-years",
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.get(
            "/api/v3/stocks/goods-report-years/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_bad_request(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v3/stocks/goods-report-years?invalid_param=value",
            headers=auth_headers['valid']
        )
    ...
    async def test_unsupported_media_type(self, async_client, auth_headers):
        ...
        pytest.skip("Not applicable for GET")
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/stocks/goods-report-years/../../etc/passwd",  # Path traversal
            "/api/v3/stocks/goods-report-years/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/stocks/goods-report-years/';DROP TABLE users;--",  # SQL injection
        ]
