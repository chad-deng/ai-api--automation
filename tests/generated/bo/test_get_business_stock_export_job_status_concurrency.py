...
# Basic Concurrency Tests for: Retrieves the status of stock export jobs for goods reports
# Method: GET
# Path: /api/v3/stocks/goods-report-years
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_business_stock_export_job_status_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/api/v3/stocks/goods-report-years", json=payload, headers=auth_headers)
