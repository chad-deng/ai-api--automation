"""
Error Scenarios Test Suite for Health check endpoint
...
class TestHealthcheckendpointErrorScenarios:
    """
    Error scenario test suite for Health check endpoint
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.get(
            "/health/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            ...
            "/health/<script>alert(1)</script>",  # XSS attempt
            "/health/';DROP TABLE users;--",  # SQL injection
        ]
