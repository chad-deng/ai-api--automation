"""
Error Scenarios Test Suite for Get subscription information
...
class TestGetsubscriptioninformationErrorScenarios:
    """
    Error scenario test suite for Get subscription information
    """
    ...
    async def test_not_found(self, async_client, auth_headers):
        """Test 404 Not Found"""
        response = await async_client.get(
            "/api/v3/subscription/nonexistent_resource_12345",
            headers=auth_headers['valid']
        )
    ...
    async def test_bad_request(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v3/subscription?invalid_param=value",
            headers=auth_headers['valid']
        )
    ...
    async def test_invalid_path_parameters(self, async_client, auth_headers):
        ...
        invalid_paths = [
            "/api/v3/subscription/../../etc/passwd",  # Path traversal
            "/api/v3/subscription/<script>alert(1)</script>",  # XSS attempt
            "/api/v3/subscription/';DROP TABLE users;--",  # SQL injection
        ]
