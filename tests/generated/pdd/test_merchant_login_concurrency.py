...
# Basic Concurrency Tests for: Authenticate merchant and return JWT token
# Method: POST
...
class TestConcurrencyEndpoint:
    ...
    async def test_merchant_login_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/auth/merchant/login", json=payload, headers=auth_headers)
