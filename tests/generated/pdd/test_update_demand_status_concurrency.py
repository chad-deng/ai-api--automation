...
# Basic Concurrency Tests for: Update demand status (Master only)
# Method: PUT
...
class TestConcurrencyEndpoint:
    ...
    async def test_update_demand_status_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.put("/demands/{id}/status", json=payload, headers=auth_headers)
