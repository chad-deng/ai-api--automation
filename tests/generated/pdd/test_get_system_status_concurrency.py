...
# Basic Concurrency Tests for: Get current system health status (Master only)
# Method: GET
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_system_status_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/admin/system/status", json=payload, headers=auth_headers)
