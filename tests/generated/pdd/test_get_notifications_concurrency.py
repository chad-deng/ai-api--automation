...
# Basic Concurrency Tests for: Get list of notifications for the current user
# Method: GET
# Path: /notifications
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_notifications_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/notifications", json=payload, headers=auth_headers)
