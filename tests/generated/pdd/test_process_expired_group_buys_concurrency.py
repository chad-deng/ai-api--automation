...
# Basic Concurrency Tests for: Manually trigger processing of expired group buys (Master only)
# Method: POST
# Path: /admin/system/process-expired-group-buys
...
class TestConcurrencyEndpoint:
    ...
    async def test_process_expired_group_buys_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/admin/system/process-expired-group-buys", json=payload, headers=auth_headers)
