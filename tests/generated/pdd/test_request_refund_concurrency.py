...
# Basic Concurrency Tests for: Request a refund for an order (for testing purposes)
# Method: POST
...
class TestConcurrencyEndpoint:
    ...
    async def test_request_refund_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/payment/refund/{orderId}", json=payload, headers=auth_headers)
