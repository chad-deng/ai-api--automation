...
# Basic Concurrency Tests for: Manually trigger a specific cron job (Master only)
# Method: POST
# Path: /admin/cron/{jobName}/trigger
...
class TestConcurrencyEndpoint:
    ...
    async def test_trigger_cron_job_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/admin/cron/{jobName}/trigger", json=payload, headers=auth_headers)
