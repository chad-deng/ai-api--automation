...
# Basic Concurrency Tests for: Mark all notifications as read for the current user
# Method: PUT
...
class TestConcurrencyEndpoint:
    ...
    async def test_mark_all_notifications_as_read_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.put("/notifications/read-all", json=payload, headers=auth_headers)
