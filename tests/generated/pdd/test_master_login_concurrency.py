...
# Basic Concurrency Tests for: Authenticate master user and return JWT token
# Method: POST
...
class TestConcurrencyEndpoint:
    ...
    async def test_master_login_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/auth/master/login", json=payload, headers=auth_headers)
