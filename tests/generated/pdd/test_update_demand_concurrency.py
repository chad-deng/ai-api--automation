...
# Basic Concurrency Tests for: Update a demand (Merchant can update own demands, Master can update any)
# Method: PUT
# Path: /demands/{id}
...
class TestConcurrencyEndpoint:
    ...
    async def test_update_demand_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.put("/demands/{id}", json=payload, headers=auth_headers)
