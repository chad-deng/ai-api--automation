...
# Basic Concurrency Tests for: Update an existing delivery address for the merchant
# Method: PUT
...
class TestConcurrencyEndpoint:
    ...
    async def test_update_merchant_address_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.put("/merchants/addresses/{addressId}", json=payload, headers=auth_headers)
