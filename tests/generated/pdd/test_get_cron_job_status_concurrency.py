...
# Basic Concurrency Tests for: Get status of all cron jobs (Master only)
# Method: GET
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_cron_job_status_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/admin/cron/status", json=payload, headers=auth_headers)
