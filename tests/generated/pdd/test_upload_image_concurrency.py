...
# Basic Concurrency Tests for: Upload an image file (Master only)
# Method: POST
# Path: /upload/image
...
class TestConcurrencyEndpoint:
    ...
    async def test_upload_image_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/upload/image", json=payload, headers=auth_headers)
