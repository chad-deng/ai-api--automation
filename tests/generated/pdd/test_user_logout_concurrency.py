...
# Basic Concurrency Tests for: Logout user and invalidate JWT token
# Method: POST
# Path: /auth/logout
...
class TestConcurrencyEndpoint:
    ...
    async def test_user_logout_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/auth/logout", json=payload, headers=auth_headers)
