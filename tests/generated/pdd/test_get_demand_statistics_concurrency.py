...
# Basic Concurrency Tests for: Get demand statistics (Master only)
# Method: GET
# Path: /demands/stats
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_demand_statistics_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/demands/stats", json=payload, headers=auth_headers)
