...
# Basic Concurrency Tests for: Create a new group buy (Master only)
# Method: POST
...
class TestConcurrencyEndpoint:
    ...
    async def test_create_group_buy_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/master/group-buys", json=payload, headers=auth_headers)
