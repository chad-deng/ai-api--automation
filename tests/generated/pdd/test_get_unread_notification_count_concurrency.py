...
# Basic Concurrency Tests for: Get the count of unread notifications for the current user
# Method: GET
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_unread_notification_count_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/notifications/unread-count", json=payload, headers=auth_headers)
