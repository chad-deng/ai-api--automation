...
# Basic Concurrency Tests for: Refresh an expired JWT token using a refresh token
# Method: POST
# Path: /auth/refresh
...
class TestConcurrencyEndpoint:
    ...
    async def test_refresh_jwt_token_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/auth/refresh", json=payload, headers=auth_headers)
