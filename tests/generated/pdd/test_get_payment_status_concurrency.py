...
# Basic Concurrency Tests for: Check the payment status of an order
# Method: GET
...
class TestConcurrencyEndpoint:
    ...
    async def test_get_payment_status_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.get("/payment/status/{orderId}", json=payload, headers=auth_headers)
