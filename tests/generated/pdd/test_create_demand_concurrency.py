...
# Basic Concurrency Tests for: Create a new product demand (Merchant only)
# Method: POST
# Path: /demands
...
class TestConcurrencyEndpoint:
    ...
    async def test_create_demand_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/demands", json=payload, headers=auth_headers)
