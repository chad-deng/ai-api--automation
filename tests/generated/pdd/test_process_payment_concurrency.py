...
# Basic Concurrency Tests for: Process payment for a group buy order
# Method: POST
# Path: /payment/pay
...
class TestConcurrencyEndpoint:
    ...
    async def test_process_payment_basic_concurrency(self, client, auth_headers):
        ...
        async def make_request(user_id: int, request_id: int):
            ...
            try:
                response = await client.post("/payment/pay", json=payload, headers=auth_headers)
