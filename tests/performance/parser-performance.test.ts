...
import { OpenAPIValidator } from '../../src/validator/openapi-validator';
...
describe('Parser Performance', () => {
  ...
  describe('Parsing Performance', () => {
    ...
    test('should handle multiple parallel parsing operations', async () => {
      ...
      const operations = Array(10).fill(null).map(() => 
        parser.parseFromFile(petstoreFixture)
      );
  ...
  // Helper functions
  async function measureParsingTime(spec: string): Promise<number> {
  ...
  function generateSmallSpec(): string {
  ...
  function generateMediumSpec(): string {
  ...
  async function generateLargeOpenAPISpec(): Promise<string> {
    const paths: Record<string, any> = {};
